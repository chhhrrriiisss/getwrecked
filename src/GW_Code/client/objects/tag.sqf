...
_obj setVariable ["tag", _obj addAction ["", "", "", 0, false, false, "", "
	    ...
	    if (!alive _target) exitWith { _target removeAction (_target getVariable 'tag'); true };
	    if ( (vehicle player) != player || player distance _target > 5) exitWith { true };	    	
	  	if (player getVariable ['isEditing', false] ) exitWith { true };
